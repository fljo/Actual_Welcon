...
ErrHandler:
  ...
  Public Function Analyze_Func(AnaType As String) As AnaResult
    ...
    Dim ScanNr_Diff As Integer

    On Error GoTo errhandler

    If ValidMeas < 1000 Then
    ...
    If AnaType = "full" Then
      ...
      If (FoundLine(1).SlutPkt.X - FoundLine(2).SlutPkt.X) > 25 Then
        LineOffs = 10
      Else
        LineOffs = 0
      End If
      FoundLine(3) = FindLinje(FoundLine(1).SlutPkt.Nr + LineOffs, ValidMeas - 100, 1, 0.3, 1, 15, 40)
      FoundLine(3) = FindEnd(FoundLine(3), -1, 0.1)
      FoundLine(3).StartPkt = FoundLine(1).SlutPkt
      MakeLine(FoundLine(3), Color.Green)


      FoundLine(4) = FindLinje(FoundLine(2).SlutPkt.Nr - LineOffs, 100, -1, 0.3, 1, 15, 40)
      FoundLine(4) = FindEnd(FoundLine(4), 1, 0.1)
      FoundLine(4).StartPkt = FoundLine(2).SlutPkt
      MakeLine(FoundLine(4), Color.Green)

      ScanNr_Diff = (FoundLine(4).SlutPkt.Nr - FoundLine(3).SlutPkt.Nr)
      If (ScanNr_Diff > 20) Then
